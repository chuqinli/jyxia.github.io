var callback = function() {
	...
	function AnimateLightBox( poplink, postWidth ) {
		...
		if ( postWidth ) {
			screenWidth = ($(window).width() * contentWidth) > 770 
							?  770 : $(window).width() * contentWidth;
		} else {
			screenWidth = ($(window).width() * contentWidth) > 970 
							?  970 : $(window).width() * contentWidth;
		}
		...
		$('.overlay').animate({ opacity : '1' },'fast', function(){
			$('.overlay-container')
				.append('<div class="popup-back load-lightbox" /><div class="close-btn">' + 
							'<span class="left"></span><span class="right"></span></div><div class="popup" /> ');
			$.ajax({
				url: poplink,
				data: {},
				cache: false,
